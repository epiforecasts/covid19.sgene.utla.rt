...
utla_rt_cases <- utla_rt %>%
  ...
  filter(week_infection >= "2020-10-01", !is.na(prop))
...
plot_rt <- function(rt_data) {
  ggplot(rt_data, aes(x = prop, y = rt_mean,
...
suppressWarnings(plot_rt(rt))
```

```{r load-models, echo = FALSE}
models <- readRDS(here::here("output", "model_comparison.rds"))
fits <- list()
fits <- readRDS(here::here("output", paste0("fits.rds")))
res <- fits %>%
  .$models %>%
  .$dynamic %>%
  .$covariates_random_region

effect_size <- function(gt, model, pos = 2) {
  paste0(models$post[[model]][pos] * 100,"%")
}
```
...
estimates <- list()
samples <- lapply(names(fits$models$static), function(model) {
  lapply(seq_along(fits$models$static[[model]]),
         function(i) {
           posterior_samples(fits$models$static[[model]][[i]]) %>%
             as_tibble() %>%
...
Using data from the `r start_date` to the `r end_date` we found consistent evidence of an association between S-gene positivity and increased UTLA level expected reproduction number estimates. The association became more apparent over time. The outer 90% credible intervals of the estimates in the time-sliced analysis (Fig. 2) ranged from a `r round((min(estimates$low) - 1) * 100 )`% increase to a `r round((max(estimates$high) - 1) * 100 )`% increase, in April/May, depending on the assumption about generation times and the model used, as the proportion of tests that were S-gene positive increased heterogeneously across NHS regions. Out of the models fitted to all time, ones that adjusted for residual variation over time on both a national and NHS region level fit the data better than those that did not (Tab. 1). However, all models had evidence of increased $R_t$ with S-gene positivity with the best fitting model yielding a lower bound of `r effect_size("short","covariates_time_by_random_region", 1)` higher $R_t$ of S-gene positive cases with a short generation time (Tab. 2), higher than the model that only adjusted for national level residual variation over time (lower bound: `r effect_size("short","covariates_time_region_random", 1)`) and lower than the model that did not adjust for residual variation over time (lower bound: `r effect_size("short","covariates_random_region", 1)`). With a longer generation time, these lower bounds changed to `r effect_size("long","covariates_time_by_random_region", 1)`, `r effect_size("long","covariates_time_region_random", 1)`, and `r effect_size("long","covariates_random_region", 1)`, respectively. The upper bound of the increase in $R_t$ varied from `r effect_size("short","covariates_time_by_random_region", 3)` to `r effect_size("long","covariates_random_region", 3)` in models with different assumed generation times.
...
st <- lapply(models$post, function(y) {
    paste0(y[1],  "--",  y[3])
})

full_models <-
  c(`Regional static` = "covariates_random_region",
    `National time-varying` = "covariates_time_region_random",
    `Regional time-varying` = "covariates_time_by_random_region")
```
...
fn <- list()
lc <- loo::loo_compare(models$loos[full_models])
fn <-
...
fn <-
  tibble::tibble(Model = names(full_models),
                 Estimate = unlist(st[full_models]))
...
plot_pred_int <- function(gt, model,
  ...
  y <- models$y
  yrep <- models$yrep[[model]]
  psis <- models$psis[[model]]
...
p <- plot_pred_int("long", "covariates_random_region")
```
...
- `data/by_utla.rds`: Weekly test positivity data for the S-gene by UTLA.
