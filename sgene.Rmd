...
Investigate the changes in the proportion of positive cases that were S-gene negative and correlate them with changes in the estimated reproduction number at the level of lower tier local authorities (LTLA).
...
The model was implemented using the `brms` [@brms] package in `R`. We used a Gaussian observation model with a single variance.
