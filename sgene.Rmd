...
rt_weekly <- rt_by_ltla %>%
  ...
  select(-n) %>%
  group_by(ltla_name, week_infection) %>%
  summarise_if(is.numeric, mean, .groups = "drop")

sgene_by_ltla <- readRDS(file.path(processed_path, "sgene_by_ltla.rds"))

by_ltla_rt <- sgene_by_ltla %>%
...
cat(sep = "",
    ...
    format(min(by_ltla_rt$week_infection), "%d %B, %Y"),
...
fn %>%
  ...
  kable_styling()
```
*Table 2: Parameter $\alpha$ with 95% credible intervals for the three different models of $s(s)$ for short (3.6 days mean) and long (5.5 days mean) generation intervals. The estimate corresponds to the multiplicative increase in reproduction number estimated for S-gene negative cases*
