...
#' Uncertainty is also included by modelling variant samples (`sgtf`) as a
...
variant_rt <- function(log_rt = ~ 1, data, brm_fn = brm, ...) {
  ...
  # define sgtf observation model
  sgtf_obs_model <- bf(sgtf_samples | trials(samples) ~ 0 + mi(sgtf)) +
    binomial(link = "identity")

  # define model for latent sgtf
  sgtf_model <- bf(sgtf | mi() ~ 1, family = "beta")
  ...
  priors <- c(
    ...
    prior(constant(1), resp = "sgtfsamples")
  )

# fit model
brm_out <- do.call(brm_fn, c(
  list(
    mvbf(sgtf_obs_model, sgtf_model, rt_model, rescor = FALSE),
    data = data,
    prior = priors),
  ...)
)
