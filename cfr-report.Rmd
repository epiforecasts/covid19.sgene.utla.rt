...
assoc <- readRDS(here("output", "associations.rds"))
```
...
```{r cfr_r_vs_prop, echo=FALSE, fig.width=10, fig.height=10}
prop_data <- cfr$data %>%
...
lc <- tibble(model = names(cfr$loos$lc[["all"]][, 1]),
             `ELPD difference` = round(cfr$loos$lc[["all"]][, "elpd_diff"], 2)) %>%
...
```{r cfr_regression_log_coeff_table, echo = FALSE, results = 'asis'}
to_per <- function(var) {
  paste0(var)
}
effects <- lc %>%
...
```{r chr_r_vs_prop, echo=FALSE, fig.width=10, fig.height=10}
prop_data <- cfr$data %>%
  mutate(cfr = deaths / cases)
...
suppressWarnings(ggplot(prop_data, aes(x = prop_sgtf, y = cfr,
  ...
  scale_y_continuous(labels = percent) +
  xlab("Proportion with S gene dropped") +
  ylab("Case hospitalisation rate") +
  theme_cowplot() +
  labs(size = paste("Hospitalisations")) +
  theme(legend.position = "bottom",
        legend.direction = "horizontal",
        legend.box = "vertical"))
```
*Figure 2: Proportion with S gene dropped compared to the adjusted case-hospitalisation rate each week beginning Monday the 5th of October. Each point represents one UTLA, with the size given by the number of hospitalisations by date of infection.*
...
lc %>%
  ...
  kable_styling()
```
*Table 3: Model comparison by difference in expected log-predictive density. Models are identified by the covariates they include.*


```{r chr_regression_log_coeff_table, echo = FALSE, results = 'asis'}
to_per <- function(var) {
  paste0(var)
}
effects <- lc %>%
...
effects %>%
  ...
  kable_styling()
```
*Table 4: Parameter $\alpha$ with 95% credible intervals for the both additive and multiplicative assumptions across all models evaluated. In the additive model the effect can be interpreted as a direct change in the case-hospitalisation rate related to S-gene negativity whilst in multiplicative model the effect can be interpreted as a scaling of the S-gene positive case-hospitalisation rate.*
...
```{r hfr_r_vs_prop, echo=FALSE, fig.width=10, fig.height=10}
prop_data <- cfr$data %>%
  mutate(cfr = deaths / cases)
...
suppressWarnings(ggplot(prop_data, aes(x = prop_sgtf, y = cfr,
  ...
  scale_y_continuous(labels = percent) +
  xlab("Proportion with S gene dropped") +
  ylab("Hospitalisation-fatality rate") +
  theme_cowplot() +
  labs(size = paste("Deaths")) +
  theme(legend.position = "bottom",
        legend.direction = "horizontal",
        legend.box = "vertical"))
```
*Figure 3: Proportion with S gene dropped compared to the adjusted hospitalisation-fatality rate each week beginning Monday the 5th of October. Each point represents one UTLA, with the size given by the number of deaths by date of infection.*
...
lc %>%
  ...
  kable_styling()
```
*Table 5: Model comparison by difference in expected log-predictive density. Models are identified by the covariates they include.*


```{r hfr_regression_log_coeff_table, echo = FALSE, results = 'asis'}
to_per <- function(var) {
  paste0(var)
}
effects <- lc %>%
...
effects %>%
  ...
  kable_styling()
```
*Table 6: Parameter $\alpha$ with 95% credible intervals for the both additive and multiplicative assumptions across all models evaluated. In the additive model the effect can be interpreted as a direct change in the hospitalisation-fatality rate related to S-gene negativity whilst in multiplicative model the effect can be interpreted as a scaling of the S-gene positive hospitalisation-fatality rate.*
